...
export function Resultados() {
  return (
    <section id="resultados" className="w-full bg-white py-20 md:py-28">
      <div className="max-w-5xl mx-auto px-6">
        <div className="reveal max-w-2xl">
          ...
          <p className="mt-5 font-sans text-brand-gray leading-relaxed">{resultados.remate}</p>
        </div>
        ...
        <RevealStagger as="ul" className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 list-none">
          {resultados.bullets.map((b) => (
            <li key={b.titulo} className="border-t-2 border-brand-accent pt-4">
              <p className="font-sans font-bold text-brand-black">{b.titulo}</p>
              <p className="mt-1.5 font-sans text-sm text-brand-gray leading-relaxed">{b.body}</p>
              {b.placeholder && <p className="mt-1.5 font-sans text-xs italic text-brand-gray/60">{b.placeholder}</p>}
            </li>
          ))}
        </RevealStagger>

        <p className="reveal mt-12 font-serif italic text-brand-black text-lg md:text-xl max-w-2xl">{resultados.honesty}</p>
