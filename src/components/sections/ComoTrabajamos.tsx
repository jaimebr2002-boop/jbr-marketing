  ...
  return (
    <section id="como-trabajamos" className="w-full bg-canvas py-20 md:py-28">
      <div className="max-w-5xl mx-auto px-6">
        <h2 className="reveal font-sans font-bold text-brand-black leading-[1.15] text-[clamp(1.75rem,4vw,2.5rem)] max-w-2xl text-balance">
        ...
        <RevealStagger as="ol" className="mt-14 grid grid-cols-1 md:grid-cols-4 gap-6 list-none">
          {comoTrabajamos.pasos.map((p, i) => (
            <li key={p.titulo} className="flex flex-col gap-2">
              <span className="font-sans text-xs font-bold uppercase tracking-[0.1em] text-brand-accent bg-brand-black inline-block w-fit px-2 py-1 rounded">
                {i + 1}
              </span>
              <p className="font-sans font-bold text-brand-black mt-1">{p.titulo}</p>
              <p className="font-sans text-xs uppercase tracking-[0.06em] text-brand-gray">{p.plazo}</p>
              <p className="font-sans text-sm text-brand-gray leading-relaxed">{p.body}</p>
            </li>
          ))}
        </RevealStagger>
      </div>
