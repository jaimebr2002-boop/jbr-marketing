        ...
        <RevealStagger as="div" className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {featured.map((s) => (
            <Link
              key={s.slug}
              to={`/servicios/${s.slug}`}
              className="group bg-surface rounded-card border border-brand-border p-6 flex flex-col gap-4 transition-all duration-200 ease-out hover:-translate-y-1 hover:border-brand-accent hover:shadow-[0_10px_28px_var(--accent-glow)] active:scale-[0.98]"
            >
